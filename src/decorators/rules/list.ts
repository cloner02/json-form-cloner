...
import { type IRulesRange, type IRulesBase } from '../../components/cinput/type/index'
import { type IRulesCondition } from '../rules/type/index'

const pattern: IRulesCondition = {
  name: 'pattern',
  condition: (rule: IRulesBase, element: CBase) => {
    return rule?.pattern !== undefined && !new RegExp(rule.pattern).test(element.value as string)
  },
  message: (rule: IRulesBase) => `Value does not match the pattern ${rule.pattern}`
}
...
const minlength: IRulesCondition = {
  ...
  message: (rule: IRulesBase) => `Value is too short the minimum is ${rule.minlength} characters`
}
...
const maxlength: IRulesCondition = {
  ...
  message: (rule: IRulesBase) => `Value is too long the maximum is ${rule.maxlength} characters`
}
